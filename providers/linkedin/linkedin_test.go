...
func TestGetUser(t *testing.T) {
  ...
  "first_name": "Mat",
  "last_name": "Ryer",
  "link": "https://www.linkedin.com/matryer",
  "username": "loginname",
  ...
  "picture": {
	...
	if assert.NoError(t, err) && assert.NotNil(t, user) {
		...
		assert.Equal(t, user.Data()["link"], "https://www.linkedin.com/matryer")

		linkedinCreds := user.ProviderCredentials()[linkedinName]
		if assert.NotNil(t, linkedinCreds) {
			assert.Equal(t, "631819186", linkedinCreds.Get(common.CredentialsKeyID).Str())
		}
...
func TestNewlinkedin(t *testing.T) {
	...
	if assert.NotNil(t, g) {
		...
		if assert.NotNil(t, g.config) {
			...
			assert.Equal(t, linkedinDefaultScope, g.config.Get(oauth2.OAuth2KeyScope).Data())

			assert.Equal(t, linkedinAuthURL, g.config.Get(oauth2.OAuth2KeyAuthURL).Data())
			assert.Equal(t, linkedinTokenURL, g.config.Get(oauth2.OAuth2KeyTokenURL).Data())
...
func TestlinkedinTripperFactory(t *testing.T) {
...
func TestlinkedinName(t *testing.T) {
	g := New("clientID", "secret", "http://myapp.com/")
	assert.Equal(t, linkedinName, g.Name())
}

func TestlinkedinGetBeginAuthURL(t *testing.T) {
	...
	if assert.NoError(t, err) {
		...
		assert.Contains(t, url, "scope="+linkedinDefaultScope)
	...
	if assert.NoError(t, err) {
		assert.Contains(t, url, "client_id=clientID")
		assert.Contains(t, url, "redirect_uri=http%3A%2F%2Fmyapp.com%2F")
		assert.Contains(t, url, "scope=avatar+"+linkedinDefaultScope)
		assert.Contains(t, url, "access_type="+oauth2.OAuth2AccessTypeOnline)
		assert.Contains(t, url, "approval_prompt="+oauth2.OAuth2ApprovalPromptAuto)
	}
