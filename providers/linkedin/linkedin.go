...
const (
	linkedinDefaultScope    string = "r_basicprofile r_emailaddress"
	linkedinName            string = "linkedin"
	linkedinDisplayName     string = "linkedin"
	linkedinAuthURL         string = "https://www.linkedin.com/uas/oauth2/authorization"
	linkedinTokenURL        string = "https://www.linkedin.com/uas/oauth2/accessToken"
	linkedinEndpointProfile string = "https://api.linkedin.com/v1/people/~:(id,formatted-name,picture-url,email-address)?format=json"
)

// linkedinProvider implements the Provider interface and provides linkedin
...
func New(clientId, clientSecret, redirectUrl string) *linkedinProvider {

	p := new(linkedinProvider)
	p.config = &common.Config{Map: objx.MSI(
		oauth2.OAuth2KeyAuthURL, linkedinAuthURL,
		oauth2.OAuth2KeyTokenURL, linkedinTokenURL,
		oauth2.OAuth2KeyClientID, clientId,
		oauth2.OAuth2KeySecret, clientSecret,
		oauth2.OAuth2KeyRedirectUrl, redirectUrl,
		oauth2.OAuth2KeyScope, linkedinDefaultScope,
...
func (provider *linkedinProvider) TripperFactory() common.TripperFactory {
...
func (provider *linkedinProvider) PublicData(options map[string]interface{}) (interface{}, error) {
	return gomniauth.ProviderPublicData(provider, options)
}

// Name is the unique name for this provider.
func (provider *linkedinProvider) Name() string {
	return linkedinName
}

// DisplayName is the human readable name for this provider.
func (provider *linkedinProvider) DisplayName() string {
	return linkedinDisplayName
}
...
func (provider *linkedinProvider) GetBeginAuthURL(state *common.State, options objx.Map) (string, error) {
	if options != nil {
		scope := oauth2.MergeScopes(options.Get(oauth2.OAuth2KeyScope).Str(), linkedinDefaultScope)
...
func (provider *linkedinProvider) Get(creds *common.Credentials, endpoint string) (objx.Map, error) {
...
func (provider *linkedinProvider) GetUser(creds *common.Credentials) (common.User, error) {

	profileData, err := provider.Get(creds, linkedinEndpointProfile)
...
func (provider *linkedinProvider) CompleteAuth(data objx.Map) (*common.Credentials, error) {
...
// protected linkedin resources
func (provider *linkedinProvider) GetClient(creds *common.Credentials) (*http.Client, error) {
